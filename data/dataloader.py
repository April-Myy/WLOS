...
def train_dataloader(path, img_size=256, batch_size=64, num_workers=0):
    image_dir = os.path.join(path, 'train')
    image_dir = path
    dataloader = DataLoader(
        Dataset_(image_dir, ps=img_size),
...
def test_dataloader(path, batch_size=1, num_workers=0):
    ...
    image_dir = path
    dataloader = DataLoader(
        Dataset_(image_dir, is_test=True),
        batch_size=batch_size,
        shuffle=False,
        num_workers=num_workers,
        pin_memory=True
...
def valid_dataloader(path, batch_size=1, num_workers=0):
    image_dir = path
    dataloader = DataLoader(
        Dataset_(image_dir, is_valid=True),
        batch_size=batch_size,
        shuffle=False,
        num_workers=num_workers
    )
...
class Dataset_(Dataset):
    ...
    def _check_image(lst):
        for x in lst:
            splits = x.split('.')
            if splits[-1] not in ['png', 'jpg', 'jpeg', 'PNG', 'JPG', 'JPEG']:
                raise ValueError
