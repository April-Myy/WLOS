...
def _train(model, args):
    ...
    model, optimizer, dataloader, ots = accelerator.prepare(
        ...
        train_dataloader(args.data_dir, args.img_size, args.batch_size, args.num_worker),
    ...
    log_dir = f"runs/{args.model_name}_lr{args.learning_rate}"
    writer = SummaryWriter(log_dir) if accelerator.is_local_main_process else None
    ...
    for epoch_idx in range(epoch, args.num_epoch + 1):
        ...
        if accelerator.is_local_main_process and epoch_idx % args.valid_freq == 0:
            ...
            model.eval()
            val = _valid(model.module if hasattr(model, 'module') else model, args, epoch_idx, ots)
